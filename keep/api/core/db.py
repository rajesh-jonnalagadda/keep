...
def get_workflows_with_last_executions_v2(tenant_id: str, fetch_last_executions: int = 15) -> List[dict]:
    ...
    with Session(engine) as session:
        ...
        workflows_with_last_executions_query = (
            select(
                Workflow,
                latest_executions_subquery.c.started.label("execution_started"),
                latest_executions_subquery.c.execution_time.label("execution_time"),
                latest_executions_subquery.c.status.label("execution_status"),
                latest_executions_subquery.c.workflow_id.label("workflow_id"),
            )
            .outerjoin(
                latest_executions_subquery,
                and_(
